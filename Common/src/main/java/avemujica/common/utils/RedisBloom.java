...
public class RedisBloom {
    ...
    public Boolean deleteBloomFilter(String key, long expectedInsertions, double falseProbability) {
        Boolean keyExist = redisTemplate.hasKey(key);
        if(keyExist) {
            RedisScript<Boolean> script = new DefaultRedisScript<>(delInBloomLua(), Boolean.class);
            RedisSerializer stringSerializer = redisTemplate.getStringSerializer();
            redisTemplate.execute(script, stringSerializer, stringSerializer, Collections.singletonList(key));
        }
        return true;
    }
    // 添加元素
